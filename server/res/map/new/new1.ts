...
export default class MapRoom extends ROOM {
    ...
    constructor() {
        ...
        this.set_item("yizi", "椅子", "这是一张椅子，可以坐。", [[
            "zuo2", "坐下去", function (this: MapRoom, me: CHARACTER) {
                ...
                var newLevel = me.query_temp<number>('new', 0);
                if (newLevel === 3) {
                    me.set_temp("new", 4);
    ...
    on_enter(me: CHARACTER) {
        ...
        if (lv == 1) {
            ...
            if (this.items.length > 0 && this.items[0]) {
                const npcItem = this.items[0];
                if ('add_action' in npcItem && 'do_command' in npcItem) {
                    npcItem.add_action("guid", "我需要指引", function (me: CHARACTER) { on_guide(me); });
                }
            }
        }
    }
    on_leave(me: CHARACTER, dir: string) {
        ...
        var lv = me.query_temp("new");
        if (!lv) return true;
        if ((lv as number) < 4) return me.notify_fail("你先听完课再乱跑。");
        if (dir != "east") return me.notify_fail("你要去东面的训练室找指引者。");
    }
}

function on_guide(me: CHARACTER) {
    var lv = me.query_temp("new");
    if (lv === 1) {
        me.notify("指引者双手抱拳，对你作了个揖道：这位" + me.call()
            + "请了，接下来我来告诉你一些这个世界的基本知识。");
...
function next_guide4(this: ROOM, me: CHARACTER) {
    ...
    if (npc) {
        if ('do_command' in npc) {
            (npc as CHARACTER).do_command("go", "east");
        }
    }
}
