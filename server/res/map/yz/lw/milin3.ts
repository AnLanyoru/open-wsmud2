...
export default class MapRoom extends ROOM {
    ...
    is_killlang(me: CHARACTER) {
        var rm = ROOM.Get('yz/lw/milin2');
        if (!rm) {
            return 0;
        }
        var actualRm = rm.query_copy2(me);
        if (!actualRm) {
            return 0;
        }
        var lang = 0;
        for (var i = 0; i < actualRm.items.length; i++) {
            if (typeof actualRm.items[i].is === 'function' && actualRm.items[i].is('yz/lw/lang')) {
                lang++;
            }
        }
        return lang;
    }
    create_lw(me: CHARACTER, npc: any, type: number, msg: string): void {
    ...
    on_lwdie(this: MapRoom, me: CHARACTER, t: number) {
        var type = "zq" + t;
