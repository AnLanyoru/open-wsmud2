...
export default class extends NPC {
    ...
    constructor() {
        ...
        this.add_action("askhyd", "活跃度查询", function (me: CHARACTER, par: string) {
            ...
            for (var i = 1; i < 7; i++) {
                ...
                str.push(lv[i]);
                str.push("活跃度：");
                str.push(String(pt.query_temp('sc' + i, 0)));
                str.push("/");
                str.push(String((pt.level + 3) * 200));
            }
            me.send(str.join(""));
            if ((me.query_temp('pt_lv', 0) ?? 0) < 5) {
        ...
        this.add_action("ptset1", "帮会设置", function (me: CHARACTER, par: string) {
            var pt = me.query_party?.();
            if (!pt) return me.notify("你还没有加入帮派。");
            if ((me.query_temp('pt_lv', 0) ?? 0) > pt.query_temp("power", 2)) {
        ...
        this.add_action("sendntc", "发布公告", function (me: CHARACTER, par: string) {

            WORLD.COMMANDS['party'].enter(me, 'notice');
        });
        this.add_action("levelup2", "设施升级", function (me: CHARACTER, par: string) {
            var pt = me.query_party?.();
            if (!pt) return me.notify("你还没有加入帮派。");
            if ((me.query_temp('pt_lv', 0) ?? 0) > 1) {
        ...
        this.add_action("jrfb", "进入副本区域", function (me: CHARACTER, par: string) {

            WORLD.COMMANDS['party'].enter(me, 'enter');
        });
    }

    on_kill(me: CHARACTER) {
    return false;
}
}
